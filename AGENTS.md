...
- When asked to make changes or write code, validate your changes by running `yarn lint`.
...
- Review and fix changes that are not yet committed or pushed before creating the pull request.
- Push with `git push -u origin HEAD` so a branch with the same name as the local branch is created on the remote.
- Use the GitHub MCP `create_pull_request` to create a pull request with the template at: https://github.com/Pinelab-studio/.github/blob/main/pull_request_template.md
