...
<a
  class="button-ghost"
  [routerLink]="['/extensions/my-plugin/my-custom-route']"
>
...
const getCustomerNameDocument = graphql(
  ...
  `,
  [addressFragment]
); // Fragments MUST be explicitly referenced
...
_must_ be explicitly referenced in every document that uses them.
