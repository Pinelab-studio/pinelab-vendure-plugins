...
export class ReviewListComponent extends TypedBaseListComponent<
...
>
  ...
  <vdr-dt2-column id="rating" heading="Rating" [sort]="sorts.get('rating')">
    <ng-template let-review="item"
      ><my-star-rating-component [rating]="review.rating"
    /></ng-template>
  </vdr-dt2-column>
...
Important: - When using `defaultVisibility`, specify the specific visible ones with `true`. _Do not_ mix
true and false values. It is implicit that any not specified will default to `false`. - The `id`, `createdAt` and `updatedAt` never need to be specified in `customizeColumns`, defaultVisibility`or`defaultColumnOrder`.
