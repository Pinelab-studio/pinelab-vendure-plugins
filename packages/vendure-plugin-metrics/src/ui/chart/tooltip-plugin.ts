  ...
  return function tooltip(chart: LineChart) {
    ...
    on('mousemove', undefined, (event: MouseEvent) => {
      ...
      points.forEach((point) =>
        point.event.element.removeClass('ct-tooltip-hover')
      );
      ...
      const formattedValue =
        meta.formatOptions.formatValueAs === 'currency'
          ...
          : new Intl.NumberFormat(meta.formatOptions.locale).format(
              +(value ?? 0)
            );

      const tooltipText = `
            <div class="tooltip-date">${dateFormatter.format(
              new Date(meta.label)
            )}</div>
