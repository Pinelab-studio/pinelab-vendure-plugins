import { EventBus, ID, Logger, PLUGIN_INIT_OPTIONS, Product, ProductEvent, ProductVariant, ProductVariantEvent, ProductVariantPrice, ProductVariantService, RequestContext, TransactionalConnection } from "@vendure/core";
...
export class BulkUpdateService implements OnApplicationBootstrap {
        ...
        this.eventBus
            ...
            .subscribe(async (event) => {
                ...
                if (this.options.enablePriceBulkUpdate && product.customFields?.price) {
                    await this.updatePriceOfVariants(ctx, product, product.customFields.price).catch((e) => {
                        Logger.error(`Error updating prices of variants for product '${product.id}': ${e?.message}`, loggerCtx);
                    });
                }
                // Bulk update custom fields
                const shouldUpdateCustomFields = this.options.bulkUpdateCustomFields.find((customFieldName) => product.customFields?.[customFieldName]);   
                if (shouldUpdateCustomFields) {
                    await this.updateCustomFieldsOfVariants(ctx, product).catch((e) => {
                        Logger.error(`Error updating custom fields of variants for product '${product.id}': ${e?.message}`, loggerCtx);
                    });
                }
            });
    }

    async updatePriceOfVariants(ctx: RequestContext, product: ProductWithCustomFields, price: number): Promise<void> {
     ...
     */
    async updateCustomFieldsOfVariants(ctx: RequestContext, updatedProduct: ProductWithCustomFields): Promise<void> {
        ...
        for (const customFieldName of this.options.bulkUpdateCustomFields) {
            if (updatedProduct.customFields?.[customFieldName]) {
                customFields[customFieldName] = updatedProduct.customFields[customFieldName];
            }
        }
        // Then we update all variants with the given custom field values
        const variants = await this.getAllVariantsForProduct(ctx, updatedProduct.id);
        const variantIds = variants.map((v) => v.id);
        const res = await this.connection
            ...
            .execute();
        Logger.info(
            `Updated custom fields '${this.options.bulkUpdateCustomFields.join(',')}' of ${res.affected} variants of product ${updatedProduct.id}`,
            loggerCtx
        );
        await this.eventBus.publish(
            new ProductVariantEvent(ctx, variants, 'updated')
    ...
    private getAllVariantsForProduct(ctx: RequestContext, productId: ID): Promise<ProductVariant[]> {
