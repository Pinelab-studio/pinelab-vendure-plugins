...
import { DefaultLogger, DefaultSearchPlugin, InitialData, LogLevel } from "@vendure/core";
...
const mockClient = {
  createTask: jest.fn().mockImplementation((request) => task = request.task.httpRequest),
  createQueue: jest.fn(),
...
describe("CloudTasks job queue e2e", () => {
  ...
  it("Should create a Queue and Task", async () => {
    ...
    ({ reindex: {id: reindexId} } = await adminClient.query(gql`mutation {reindex {id}}`));
    expect(reindexId).toBeDefined();
    expect(mockClient.locationPath).lastCalledWith("test-project", "europe-west1");
    expect(mockClient.queuePath).lastCalledWith("test-project", "europe-west1", "update-search-index-plugin-test");
  ...
  it("Should fail unauthorized webhook", async () => {
    ...
    const res = await fetch(`http://localhost:3050/cloud-tasks/handler`, {
      method: 'post',
      body: buff.toString(),
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer malicious-attempt' },
    });
  ...
  it("Should handle incoming task", async () => {
    ...
    const res = await fetch(`http://localhost:3050/cloud-tasks/handler`, {
      method: 'post',
      body: buff.toString(),
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer some-secret' },
    });
