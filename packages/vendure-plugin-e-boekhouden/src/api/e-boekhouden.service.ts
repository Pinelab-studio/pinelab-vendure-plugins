...
import { EBoekhoudenConfigEntity } from './goedgepickt-config.entity';
  ...
  implements OnApplicationBootstrap, OnModuleInit
{
  ...
  async upsertConfig(
    config: Partial<EBoekhoudenConfigEntity>
   ...
   */
  async setWebhooks(channelToken: string): Promise<EBoekhoudenConfigEntity> {
    const client = await this.getClientForChannel(channelToken);
  ...
  async createOrder(
    ...
    const { houseNumber, addition } =
      EBoekhoudenService.splitHouseNumberAndAddition(
        order.shippingAddress.streetLine2
      );
