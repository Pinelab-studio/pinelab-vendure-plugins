...
export class PostalCodeResolver {
  ...
  constructor() {
    if (!DutchPostalCodePlugin.apiKey) {
      throw Error(`DutchPostalCodePlugin needs an apiKey. Use DutchPostalCodePlugin.init('yourkey') to set an apiKey`);
    }
    this.headers = {
  ...
  async dutchAddressLookup(@Ctx() ctx: RequestContext, @Args('input') input: { postalCode: string, houseNumber: string }): Promise<DutchAddressLookupResult | undefined> {
    ...
    const result = await fetch(`https://postcode.tech/api/v1/postcode/full?postcode=${postalCode}&number=${houseNumber}`, {headers: this.headers});
