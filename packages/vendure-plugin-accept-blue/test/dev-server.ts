...
import { AcceptBluePlugin, AcceptBlueService } from '../src';
...
(async () => {
  ...
  const client = await server.app
    .get(AcceptBlueService)
    .getClientForChannel(ctx);
  console.log(await client.getCustomer('jeff_ka8+sharong@hotmail.com'));
